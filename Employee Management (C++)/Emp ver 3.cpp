...
// Update feature in this ver: + Searching Online
//                             + Checking same Employee Code
//                             +
...
bool samePrefix(string Dest, string FindText) {
   
   if (Dest.size() < FindText.size()) return false;
   for (int i = 0; i < FindText.size(); i++) {
      if ((int) Dest[i] > (int) FindText[i]) {
         if ((int) Dest[i] - 32 != (int) FindText[i]) return false;
      }
      if ((int) Dest[i] < (int) FindText[i]) {
         if ((int) FindText[i] - 32 != (int) Dest[i]) return false;
      }
   }
   return true;
...
void Find_Employee(int Type) {
   ...
   while (true) {
      ...
      if ((int) hitKey == 8) {
         if (FindText.size() > 0 && Noti.size() > minimumNotiSize) {
            FindText.erase(FindText.size() - 1, 1);
      ...
      for (int i = 1; i <= Number; i++) {
         if (Type == 1) {
            if (samePrefix(Employees[i].FirstName, FindText)) {
               curNumber++;
            ...
            if (samePrefix(CodeString, FindText)) {
               curNumber++;
               Found[curNumber] = Employees[i];
            }
         }
         else if (Type == 3) {
            if (samePrefix(Employees[i].Department, FindText)) {
               curNumber++;
               Found[curNumber] = Employees[i];
            }
         }
         else if (Type == 4) {
            if (samePrefix(Employees[i].Position, FindText)) {
               curNumber++;
               Found[curNumber] = Employees[i];
            }
         }
